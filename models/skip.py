...
def skip(num_input_channels=2, num_output_channels=3,
    ...
    for i in range(len(num_channels_down)):
        ...
        if use_interpolate:
            deeper.add(Interpolate(scale_factor=2, mode=upsample_mode[i], align_corners=align_corners))
        else:
            deeper.add(nn.Upsample(scale_factor=2, mode=upsample_mode[i], align_corners=align_corners))
